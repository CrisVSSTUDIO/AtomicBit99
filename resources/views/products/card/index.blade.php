<x-app-layout>
    <x-assest-side-bar-layout>
        ...
        <x-splade-lazy>
            ...
            @php $filextension = $asset->filetype_prediction;
            @endphp

                <x-card title="{{ $asset->name ?? 'No name' }}">
                    @if ($filextension)
                    @switch($filextension)
                    @case($filextension == 'glb')
                    <model-viewer alt="{{ $asset->name ?? 'No name' }}" src="{{ asset('storage/' . $asset->upload) }}" ar camera-controls touch-action="pan-y" loading="lazy"></model-viewer>
                    @break
                    ...
                    <link rel="import" src="{{ asset('storage/' . $asset->upload) }}" title="{{ $asset->name ?? 'No name' }}" >
                    @break
                    ...
                    @default
                    <img src="{{ asset('storage/' . $asset->upload) }}" alt="{{ $asset->name ?? 'No name' }}" class="card-img-top" loading="lazy" />
                    @endswitch
                    @endif
                    <hr>
                   
                    <span class="p-2">{{ $asset->description ?? 'No description' }}</span>
                    <div class="container flex flex-wrap align-center gap-2 md:flex-row sm:flex-col justify-center ">
                ...
                Edit </Link>
                <Link modal href="{{ route('assets.show', $asset) }}"
