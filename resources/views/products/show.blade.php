<x-app-layout>
        ...
        <x-header-section title="You are viewing {{ $asset->name }}">
            ...
            <Link slideover href="{{ route('assets.edit', $asset) }}"
                class="px-4 py-1 bg-indigo-100 border border-indigo-400 rounded-md text-indigo-600 hover:bg-indigo-200 ">
            Edit </Link>
                ...
                @switch($filextension)
                    @case($filextension == 'glb')
                        <model-viewer alt="{{ $asset->name ?? 'Sem nome' }}" src="{{ asset('storage/' . $asset->upload) }}" ar
                            camera-controls touch-action="pan-y" loading="lazy"
                            class="h-screen ,w-screen md:h-full, w-full
     "></model-viewer>
                    @break
            ...
            ">
                    ...
                    @default
                        <img src="{{ asset('storage/' . $asset->upload) }}" alt="{{ $asset->name ?? 'Sem nome' }}"
                            loading="lazy" class="h-fit w-screen
